...
public:
	...
	void updateAngleAndSpeed(double gx, double gy, vector<Boid> boids)
	{
		...
		double repel;
		double bx = nearestBoid.x == BOUNDARY * 2.0 ? 0.0 : bSpeedDirection.x;
		double by = nearestBoid.y == BOUNDARY * 2.0 ? 0.0 : bSpeedDirection.y;
